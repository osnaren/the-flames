...
export default function LetterGrid({
  ...
  const renderLetter = (letter: string, position: number, nameIndex: number) => {
    ...
    return (
      <motion.button
        key={key}
        className={`w-10 h-10 rounded-lg font-handwriting text-xl flex items-center justify-center
      ...
      >
        ...
        {isCrossed && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
          >
            <div className="w-full h-0.5 bg-red-500 dark:bg-red-400 rotate-45 transform origin-center" />
          </motion.div>
        )}
      </motion.button>
    );
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {[...name1.toLowerCase()].map((letter, position) => 
            renderLetter(letter, position, 1)
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {[...name2.toLowerCase()].map((letter, position) => 
            renderLetter(letter, position, 2)
          )}
        </div>
      </div>
    </div>
