...
import { resultMeanings } from '../../flames.utils';
...
export default function FlamesCounter({
  remainingLetters,
  onResult
}: FlamesCounterProps) {
        ...
        <AnimatePresence>
          {flames.map((letter, index) => {
            ...
            return (
              <motion.button
                key={`${letter}-${index}`}
                className={`w-12 h-12 rounded-lg font-handwriting text-xl
      ...
      {flames.length === 1 && (
        ...
        >
          <div className="text-2xl font-semibold text-gray-800 dark:text-white">
            {resultMeanings[flames[0] as FlamesResult].text}
          </div>
          <div className="text-gray-600 dark:text-gray-400 mt-2">
            {resultMeanings[flames[0] as FlamesResult].quote}
          </div>
        </motion.div>
      )}
    </div>
